...
Generator <- function(sce, phenoData, Num.mixtures = 1000, pool.size = 100, min.percentage = 1, 
                         max.percentage = 99,
                         seed = 24){ 
  ...
  return(list(T = T, P = P, cells=cells))
...
Deconvolution <- function(T, C, method, phenoDataC, P = NULL, elem = NULL, STRING = NULL, marker_distrib, refProfiles.var){ 
    ...
    if(method %in% bulk_methods){
        ...
        C = subset(C, rownames(C) %in% marker_distrib$gene)
        T = subset(T, rownames(T) %in% marker_distrib$gene)
    ...
    print(RESULTS)
    RESULTS = subset(RESULTS, rownames(RESULTS) %in% gtools::mixedsort(rownames(RESULTS)) )
    print(RESULTS)
